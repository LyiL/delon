...
export class ReuseTabComponent implements OnInit, OnChanges, OnDestroy {
    ...
    _list: { url: string, title: string, depth:number, [key: string]: any }[] = [];
    _pos = 0;
    ...
    set depth(_depth:number){
        this._depth = coerceNumberProperty(_depth);
    }
    ...
    private gen(url?: string) {
        ...
        if (this.showCurrent) {
            ...
            if (idx !== -1) {
                ...
                const menu = this.srv.getMenu(url);
                ls.push({
                    url,
    ...
    clear() {
        let tmpList = this._list['find']((w,index)=>{
            return w.depth <= this._depth;
        });
        this._list = [ tmpList ];
        this.srv.clear();
