...
export class ReuseTabService implements OnDestroy {
    ...
    refresh(){
        this._cachedChange.next({active:'refresh'});
    }

    // endregion

    constructor(private injector: Injector, @Optional() private menuService: MenuService,private logger:NzLocaleService) { }
    ...
    shouldDetach(route: ActivatedRouteSnapshot): boolean {
        ...
        this.di('#shouldDetach', this.getUrl(route), this.can(route),this.get(this.getUrl(route)));
    ...
    store(_snapshot: ActivatedRouteSnapshot, _handle: {}) {
        ...
        const idx = this.index(url);
        const menu = this.getMenu(url);
        //当进入缓存时，判断是否有还有子菜单缓存，有删除
        if(menu && menu.children && menu.children.length > 0){
            this._removeChildren(menu.children);
        }
        const item: ReuseTabCached = {
            customTitle: this._titleCached[url],
            title: this.getTitle(url, _snapshot),
            _depth:this.getMenu(url)._depth,
    ...
    shouldReuseRoute(future: ActivatedRouteSnapshot, curr: ActivatedRouteSnapshot): boolean {
        const url = this.getUrl(future);
        const _curCached = this.get(url);
        const menu = this.getMenu(url);
        if(_curCached && menu._depth > this.depth){
            this._remove(url);
            this._removeChildren(menu.children);
    ...
    ngOnDestroy(): void {
        this._cached.splice(0,this._cached.length);
        this._cached = null;
     ...
     * @private
     */
    private _removeChildren(children:Array<any>){
        if(children && children.length > 0){
            children.forEach((subChildren:any)=>{
                this._remove(subChildren.link);
                if(subChildren.children && subChildren.children.length > 0){
                    this._removeChildren(subChildren.children);
                }
            });
        }
    }
